...
test("Bailian Coding Plan owns an independent five-model media catalog", () => {
  assert.deepEqual(
...
test("Wan image generation uses only the Bailian Coding Plan endpoint and key", async () => {
  ...
  try {
    const result = await handleImageGeneration({
      body: {
        model: "bailian-coding-plan/wan2.7-image-pro",
...
test("Bailian Coding Plan rejects models outside its own media allowlists", async () => {
  const imageResult = await handleImageGeneration({
    body: {
      model: "bailian-coding-plan/qwen-image-2.0-pro",
